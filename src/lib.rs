...
pub fn get_johnston_components(x: i64, y: i64) -> Result<JohnstonComponents, String> {
    ...
    if n7 > 0 {
        while qn > 0 {
            ...
            if "GBD".contains(char) {
                pm7 += 1;
                sl += 1;
            } else if "CF".contains(char) {
                sf7 -= 1;
                sl += 1;
            } else {
                sl += 1;
        ...
        while qn < 0 {
            ...
            let char = letters7.chars().nth(k as usize).unwrap();
            if "CFA".contains(char) {
                pm7 -= 1;
                sl -= 1;
            } else if "BE".contains(char) {
                sf7 += 1;
                sl -= 1;
            } else {
                sl -= 1;
    ...
    let preshift13 = |c: char| match c {
        'C' => 0, 'A' => 1, 'F' => 2, 'D' => 3, 'B' => 4, 'G' => 5, 'E' => 6,
        _ => 0,
    };
    ...
    let name_base = letters13.chars().nth(pn13 as usize).unwrap().to_string();
    let sf = sf3 + sf5 + sf7 + sf11 + sf13;
    let pm = pm3 + pm5 + pm7 + pm11 + pm13;
...
pub fn note_name(x: i64, y: i64, quiet: bool) -> String {
    ...
    match result {
        ...
        Ok(comp) => {
            ...
            if [comp.sf, comp.pm, comp.sl, comp.ud, comp.e3].iter().any(|&v| v.abs() > 4) {
                let aka = format!("({} {} {} {} {} {})", comp.letter, comp.sf, comp.sl, comp.ud, comp.e3, comp.pm);
                if !quiet {
                    println!("{}", aka);
                }
                name = format!("{}\n{}", name, aka);
...
mod tests {
    ...
    fn test_note_name() {
        ...
        assert_eq!(note_name(31, 16, true), "Sorry - that is a 31-limit pitch!");
